...
export class GetNavItems {
  public constructor(init?: Partial<GetNavItems>) {
    (Object as any).assign(this, init);
  }
...
export class GetNavItemsResponse {
  ...
  public constructor(init?: Partial<GetNavItemsResponse>) {
    (Object as any).assign(this, init);
  }
}
...
export class SingletonInstanceResolver implements IResolver {
  tryResolve(ctor: ObjectConstructor): any {
    return (ctor as any).instance || ((ctor as any).instance = new ctor());
  }
}
...
export class ServerEventsClient {
  ...
  onMessage = (e: IOnMessageEvent) => {
    ...
    const request: ServerEventMessage = {
      eventId,
      data,
      type,
      channel,
    ...
    const mergedBody =
      typeof body == "object" ? Object.assign({}, request, body) : request;
    ...
    if (op === "cmd") {
      if (cmd === "onConnect") {
        ...
        if (typeof body.heartbeatIntervalMs == "string")
          this.connectionInfo.heartbeatIntervalMs = parseInt(
  ...
  reconnectServerEvents(opt: IReconnectServerEventsOptions = {}) {
    ...
    es.addEventListener("error", (e) =>
      (opt.onerror || hold.onerror || this.onError)(e)
    );
    es.addEventListener(
      "message",
      opt.onmessage || hold.onmessage || this.onMessage
    );
  ...
  stop(): Promise<void> {
    ...
    return fetch(
      new Request(hold.unRegisterUrl, {
        method: "POST",
        mode: "cors",
        credentials: this.serviceClient.credentials,
      })
  ...
  updateChannels(channels: string[]) {
    ...
    const url =
      this.eventSource != null ? this.eventSource.url : this.eventStreamUri;
    this.eventStreamUri =
      url.substring(0, Math.min(url.indexOf("?"), url.length)) +
      "?channels=" +
      channels.join(",") +
      "&t=" +
      new Date().getTime();
  ...
  update(subscribe: string | string[], unsubscribe: string | string[]) {
    ...
    var unsub =
      typeof unsubscribe == "string" ? unsubscribe.split(",") : unsubscribe;
    var channels = [];
...
export class UpdateEventSubscriber
  implements IReturn<UpdateEventSubscriberResponse> {
  id: string;
...
export class JsonServiceClient {
  ...
  sendAllOneWay<T>(requests: IReturn<T>[]): Promise<void> {
    ...
    return this.send<T[]>(HttpMethods.Post, requests, null, url).then(
      (r) => void 0
    );
  }
  ...
  private createRequest({
    ...
    if (this.bearerToken != null) {
      ...
      this.headers.set(
        "Authorization",
        "Basic " +
          JsonServiceClient.toBase64(`${this.userName}:${this.password}`)
      );
    }
    ...
    if (this.manageCookies) {
      ...
      if (cookies.length > 0) this.headers.set("Cookie", cookies.join("; "));
  ...
  private createResponse<T>(res: Response, request: any | null) {
    ...
    if (this.manageCookies) {
      ...
      res.headers.forEach((v, k) => {
        if ("set-cookie" == k.toLowerCase()) setCookies.push(v);
      });
    ...
    var x =
      ...
      typeof request.createResponse == "function"
  ...
  private handleError(holdRes: Response, res, type: ErrorResponseType = null) {
    ...
    if (res.bodyUsed)
      throw this.raiseError(
        res,
        createErrorResponse(res.status, res.statusText, type)
      );
      ...
      .catch((error) => {
        ...
        if (
          ...
          (typeof window != "undefined" &&
            error instanceof (window as any).DOMException) /*MS Edge*/
        ) {
          throw this.raiseError(
            res,
            createErrorResponse(res.status, res.statusText, type)
        ...
        if (res.status === 401) {
          if (this.refreshToken) {
            ...
            let url =
              ...
              this.createUrlFromDto(HttpMethods.Post, jwtReq);
            ...
            return fetch(url, jwtRequest)
              .then((r) =>
                this.createResponse(r, jwtReq).then((jwtResponse) => {
...
export const toCamelCase = (s: string) =>
  !s ? s : s.charAt(0).toLowerCase() + s.substring(1);

export const toPascalCase = (s: string) =>
  !s ? s : s.charAt(0).toUpperCase() + s.substring(1);
...
export const sanitize = (status: any): any => {
  ...
  for (let k in status) {
    if (status.hasOwnProperty(k)) {
      if (status[k] instanceof Object) to[toCamelCase(k)] = sanitize(status[k]);
      else to[toCamelCase(k)] = status[k];
    }
  }
...
export const queryString = (url: string): any => {
  ...
  for (var i = 0; i < pairs.length; ++i) {
    var p = pairs[i].split("=");
    map[p[0]] =
      p.length > 1 ? decodeURIComponent(p[1].replace(/\+/g, " ")) : null;
  }
  return map;
};
...
export const parseCookie = (setCookie: string): Cookie => {
  ...
  for (var i = 0; i < pairs.length; i++) {
    ...
    var value =
      parts.length > 1 ? tryDecode(stripQuotes(parts[1].trim())) : null;
    if (i == 0) {
      to = { name, value, path: "/" };
    } else {
...
export function errorResponseExcept(fieldNames: string[] | string) {
  ...
  if (names && !(status.errors == null || status.errors.length == 0)) {
    ...
    for (let field of status.errors) {
      if (
        lowerFieldsNames.indexOf((field.fieldName || "").toLowerCase()) !== -1
      ) {
        return undefined;
      }
    }
...
export const dateFmt = (d: Date = new Date()) =>
  d.getFullYear() + "/" + padInt(d.getMonth() + 1) + "/" + padInt(d.getDate());
export const dateFmtHM = (d: Date = new Date()) =>
  d.getFullYear() +
  "/" +
  padInt(d.getMonth() + 1) +
  "/" +
  ...
  ":" +
  padInt(d.getMinutes());
export const timeFmt12 = (d: Date = new Date()) =>
  padInt((d.getHours() + 24) % 12 || 12) +
  ":" +
  padInt(d.getMinutes()) +
  ":" +
  padInt(d.getSeconds()) +
...
export const toLocalISOString = (d: Date = new Date()) =>
  `${d.getFullYear()}-${padInt(d.getMonth() + 1)}-${padInt(
    d.getDate()
  )}T${padInt(d.getHours())}:${padInt(d.getMinutes())}:${padInt(
    d.getSeconds()
  )}`;
...
export function createElement(
  ...
  if (options && options.insertAfter) {
    options.insertAfter.parentNode!.insertBefore(
      el,
      options.insertAfter.nextSibling
    );
  }
  return el;
}
function showInvalidInputs(this: HTMLInputElement) {
  ...
  if (errorMsg) {
    ...
    const isCheck =
      this.type === "checkbox" ||
      this.type === "radio" ||
      hasClass(this, "form-check");
    const elFormCheck = isCheck ? parent(this, "form-check") : null;
    if (!isCheck) addClass(this, "is-invalid");
    else addClass(elFormCheck || this.parentElement, "is-invalid form-control");

    const elNext = this.nextElementSibling;
    const elLast =
      elNext && (attr(elNext, "for") === this.id || elNext.tagName === "SMALL")
        ? isCheck
          ? elFormCheck || elNext.parentElement
          : elNext
        : this;
    const elError =
      elLast != null &&
      elLast.nextElementSibling &&
      hasClass(elLast.nextElementSibling, "invalid-feedback")
        ? elLast.nextElementSibling
        : createElement(
            "div",
            { insertAfter: elLast },
...
if (typeof window != "undefined" && (window as any).Element !== undefined) {
  // polyfill IE9+
...
function handleEvent(handlers: any, el: Node = document, type: string) {
  el.addEventListener(type, function (evt) {
    ...
    if (!x) {
      ...
      if (elParent) x = attr(elParent, evtData);
    }
    if (!x) return;
...
function applyErrors(f: HTMLFormElement, status: any, opt: IAjaxFormOptions) {
  ...
  if (errors && errors.length) {
    ...
    $("input,textarea,select,button").forEach((x) => {
      ...
      if (!bs4) {
        if (hasClass(prev, "help-inline") || hasClass(prev, "help-block")) {
          ...
          hasClass(next, "help-inline") ||
          hasClass(next, "help-block")
        ) {
      ...
      if (field) {
        if (!bs4) {
          ...
          sattr(
            field,
            "data-invalid",
            filter(error.message, error.errorCode, "field")
          );
        }
      }
    ...
    const htmlSummary = filter(
      status.message || splitCase(status.errorCode),
      status.errorCode,
      "summary"
    );
    if (!bs4) {
      ...
      $("[data-validation-summary]").forEach(
        (el) =>
          (el.innerHTML =
            htmlSummary[0] === "<" ? htmlSummary : bsAlert(htmlSummary))
      );
    }
  }
  return f;
...
export const toVarNames = (names: string[] | string | null) =>
  !names
    ? []
    : isArray(names)
    ? (names as string[])
...
export function formSubmit(
  ...
  if (contentType == Types.MultiPart && (window as any).FormData === undefined)
    throw new Error(
      `FormData Type is needed to send '${Types.MultiPart}' Content Types`
    );
    ...
    .then((r) => {
      ...
      if (!r.ok) {
        return r
          ...
          .catch((e) => {
            throw new Error(
              "The request failed with " + (r.statusText || r.status)
            );
          })
          .then((o) => {
...
export function ajaxSubmit(f: HTMLFormElement, options: IAjaxFormOptions = {}) {
  ...
  addClass(f, "loading");
  const disableSel =
    options.onSubmitDisable == null ? "[type=submit]" : options.onSubmitDisable;
  ...
  function handleError(errMsg: string | null, err: any = null) {
    ...
    if (bs4) {
      $("[data-invalid]").forEach((el) =>
        showInvalidInputs.call(el as HTMLInputElement)
      );
    }
  }
  ...
  return submitFn
    ...
    .catch((e) => {
      if (e.responseStatus) handleError(null, e);
      else handleError(`${e.message || e}`, null);
    })
    .finally(() => {
      remClass(f, "loading");
      if (disable) {
...
function formEntries<T>(
  form: HTMLFormElement,
  state: T,
  fn: (state: T, name: string, value: string | Blob) => void
): T {
  let field,
    f: any = form;
...
export const serializeToFormData = (form: HTMLFormElement) =>
  formEntries(form, new FormData(), (to, name, value) =>
...
export function triggerEvent(el: Element, name: string, data: any = null) {
  if (document.createEvent) {
    let evt = document.createEvent(
      name == "click" || name.startsWith("mouse") ? "MouseEvents" : "HTMLEvents"
    );
...
export function populateForm(form: HTMLFormElement, model: any) {
  ...
  for (var key in model) {
    ...
    const el =
      form.elements.namedItem(key) || form.elements.namedItem(toggleCase(key));
      ...
      case "select-multiple":
        ...
        for (let i = 0; i < select.options.length; i++) {
          select.options[i].selected =
            values.indexOf(select.options[i].value) > -1;
        }
        break;
      case "select":
      case "select-one":
...
export function activeClassNav(x: NavItem, activePath: string) {
  return x.href != null &&
    (x.exact || activePath.length <= 1
      ? trimEnd(activePath, "/").toLowerCase() ===
        trimEnd(x.href, "/").toLowerCase()
      : trimEnd(activePath, "/")
          .toLowerCase()
          .startsWith(trimEnd(x.href, "/").toLowerCase()))
    ? "active"
    : null;
}
...
export function activeClass(
  ...
  return href != null &&
    (exact || activePath.length <= 1
      ? trimEnd(activePath, "/").toLowerCase() ===
        trimEnd(href, "/").toLowerCase()
      : trimEnd(activePath, "/")
          .toLowerCase()
          .startsWith(trimEnd(href, "/").toLowerCase()))
    ? "active"
    : null;
}
...
export class NavDefaults {
  ...
  public static forNav(options?: NavOptions | null) {
    return options || NavDefaults.create();
  }
  public static overrideDefaults(
    targets: NavOptions | null | undefined,
    source: NavOptions
  ) {
    if (targets == null) {
      return source;
    ...
    if (
      targets.navItemClass === NavDefaults.navItemClass &&
      source.navItemClass != null
    ) {
      targets.navItemClass = source.navItemClass;
    }
    if (
      targets.navLinkClass === NavDefaults.navLinkClass &&
      source.navLinkClass != null
    ) {
      targets.navLinkClass = source.navLinkClass;
    }
    if (
      targets.childNavItemClass === NavDefaults.childNavItemClass &&
      source.childNavItemClass != null
    ) {
      targets.childNavItemClass = source.childNavItemClass;
    }
    if (
      targets.childNavLinkClass === NavDefaults.childNavLinkClass &&
      source.childNavLinkClass != null
    ) {
      targets.childNavLinkClass = source.childNavLinkClass;
    }
    if (
      targets.childNavMenuClass === NavDefaults.childNavMenuClass &&
      source.childNavMenuClass != null
    ) {
      targets.childNavMenuClass = source.childNavMenuClass;
    }
    if (
      targets.childNavMenuItemClass === NavDefaults.childNavMenuItemClass &&
      source.childNavMenuItemClass != null
    ) {
...
export class NavLinkDefaults {
  public static forNavLink(options?: NavOptions | null) {
    return options || NavDefaults.create();
  }
}
export class NavbarDefaults {
  ...
  public static create() {
    return new NavOptions({ navClass: NavbarDefaults.navClass });
  }
  public static forNavbar(options?: NavOptions | null) {
...
export class NavButtonGroupDefaults {
  ...
  public static forNavButtonGroup(options?: NavOptions | null) {
    return NavDefaults.overrideDefaults(
      options,
      NavButtonGroupDefaults.create()
    );
  }
}
export class LinkButtonDefaults {
  ...
  public static create() {
    return new NavOptions({ navItemClass: LinkButtonDefaults.navItemClass });
  }
  public static forLinkButton(options?: NavOptions | null) {
    return NavDefaults.overrideDefaults(
      options || null,
      LinkButtonDefaults.create()
    );
  }
}
...
export function toXsdDuration(time: number) {
  return timeFmt(time, true);
}
export function toTimeSpanFmt(time: number) {
