...
const postChat = (sse:ServerEventsClient, message:string, channel?:string) => {
    const request = new PostChatToChannel();
...
const postRaw = (sse:ServerEventsClient, selector:string, message:string, channel?:string) => {
    const request = new PostRawToChannel();
    request.from = sse.getSubscriptionId();
    request.message = message;
    request.channel = channel || ServerEventsClient.UnknownChannel;
...
const postObject = (sse:ServerEventsClient, dto:CustomType, selector?:string, channel?:string) => {
    const request = new PostObjectToChannel();
...
describe ('ServerEventsClient Tests', () => {
    ...
    it ('Can connect to ServerEventsStream', done => {
        const client = new ServerEventsClient(SERVER_EVENTS_URL, ["*"], {
            handlers: {
                onConnect: (e => {
    ...
    it ('Does fire onJoin events for multiple Channels', done => {
        ...
        const joinMsgs:ServerEventJoin[] = []; 

        const client = new ServerEventsClient(SERVER_EVENTS_URL, channels, {
            handlers: {
    ...
    it ('Does fire all callbacks', done => {
        let connectMsgs: ServerEventConnect[] = [];
        let msgs: ServerEventMessage[] = [];
        let commands: ServerEventMessage[] = [];
        let errors = [];
        ...
        let client2:ServerEventsClient;
        states.unshift(
        {   
    ...
    it ('Does receive messages', done => {
        let connectMsgs: ServerEventConnect[] = [];
        let commands: ServerEventMessage[] = [];
        let msgs1: ServerEventMessage[] = [];
        let msgs2: ServerEventMessage[] = [];

        let states = [];

        let client2 = null;
        const client1 = new ServerEventsClient(SERVER_EVENTS_URL, ["*"], {
            handlers: {
                onConnect: (e => connectMsgs.push(e)),
                onCommand: (e => commands.push(e)),
        ...
        let info1:ServerEventConnect;
        let info2:ServerEventConnect;
        ...
        states.unshift({
            ...
            fn(){
                ...
                let chatMsg1:ChatMessage = JSON.parse(msg1.json);
                let chatMsg2:ChatMessage = JSON.parse(msg2.json);
    ...
    it ('Does send multiple heartbeats', function(done) {
        ...
        let heartbeats:ServerEventHeartbeat[] = [];
    ...
    it ('Does send message to named receiver', function(done) {
        ...
        states.unshift({
            ...
            fn(){
                let foo = TestNamedReceiver.FooMethodReceived;
            ...
            fn(){
                let bar = TestNamedReceiver.BarMethodReceived;
            ...
            fn(){
                let baz = TestNamedReceiver.NoSuchMethodReceived;
                expect(baz).to.not.undefined;
                expect(baz.id).to.equal(3);
                expect(baz.name).to.equal("Baz");
                expect(TestNamedReceiver.NoSuchMethodSelector).to.equal("BazMethod");
            ...
            fn(){
                let qux = TestNamedReceiver.QuxSetterReceived;
    ...
    it ('Does send message to global receiver', done => {
        ...
        states.unshift({
            ...
            fn(){
                let foo = TestGlobalReceiver.CustomTypeMethodReceived;
                expect(foo).to.not.undefined;
                expect(foo.id).to.equal(1);
                expect(foo.name).to.equal("Foo");
    ...
    it ('Does set properties on global receiver', done => {
        ...
        states.unshift({
            ...
            fn(){
                let foo = TestGlobalReceiver.SetterTypeReceived;
                expect(foo).to.not.undefined;
                expect(foo.id).to.equal(1);
                expect(foo.name).to.equal("Foo");
    ...
    it ('Does send raw string messages', function(done) {
        ...
        states.unshift({
            ...
            fn(){
                let chatMsg = TestJavaScriptReceiver.ChatReceived;
            ...
            fn(){
                let announce = TestJavaScriptReceiver.AnnounceReceived;
            ...
            fn(){
                let toggle = TestJavaScriptReceiver.ToggleReceived;
                expect(toggle).to.null;
                let toggleRequest = TestJavaScriptReceiver.ToggleRequestReceived;
            ...
            fn(){
                let bgImage = TestJavaScriptReceiver.BackgroundImageReceived;
                expect(bgImage).to.equal("url(http://bit.ly/1yIJOBH)");
                let bgImageRequest = TestJavaScriptReceiver.BackgroundImageRequestReceived;
    ...
    it ('Can reuse same instance', done => {
        ...
        states.unshift({
            ...
            fn(){
                let instance:TestJavaScriptReceiver = client1.resolver.tryResolve(TestJavaScriptReceiver);
    ...
    it ('Does receive messages on to clients subscribed on multiple channels', done => {
        let msgsA:ServerEventMessage[] = [];
        let msgsAB:ServerEventMessage[] = [];
        let msgsABC:ServerEventMessage[] = [];
        let msgsABCD:ServerEventMessage[] = [];

        let states = [];
        const clientA = new ServerEventsClient(SERVER_EVENTS_URL, ["A"], {
            handlers: { onMessage: e => msgsA.push(e) }, onTick: run(states)
        });
        const clientAB = new ServerEventsClient(SERVER_EVENTS_URL, ["A", "B"], {
            handlers: { onMessage: e => msgsAB.push(e) }, onTick: run(states)
        });
        const clientABC = new ServerEventsClient(SERVER_EVENTS_URL, ["A", "B", "C"], {
            handlers: { onMessage: e => msgsABC.push(e) }, onTick: run(states)
        });
        const clientABCD = new ServerEventsClient(SERVER_EVENTS_URL, ["A", "B", "C", "D"], {
        ...
        let channelAsubscribers:ServerEventUser[] = [];
        let channelABsubscribers:ServerEventUser[] = [];
    ...
    it ('Does receive all join and leave messages', function(done) {
        ...
        let joinA:ServerEventJoin[] = [];
        let joinB:ServerEventJoin[] = [];
        let joinAB:ServerEventJoin[] = [];

        let leaveA:ServerEventJoin[] = [];
        let leaveB:ServerEventJoin[] = [];
        let leaveAB:ServerEventJoin[] = [];

        let states = [];
        const clientA = new ServerEventsClient(SERVER_EVENTS_URL, ["A"], {
            handlers: { onJoin: e => joinA.push(e), onLeave: e => leaveA.push(e) }, onTick: run(states)
        });
        const clientB = new ServerEventsClient(SERVER_EVENTS_URL, ["B"], {
    ...
    it ('Can subscribe to channels whilst connected', function(done) {
        ...
        let states = [];
        const client1 = new ServerEventsClient(SERVER_EVENTS_URL, ["A"], {
            handlers: { onMessage: e => msgs1.push(e) }, 
            onTick: run(states)
        }).start();
        const client2 = new ServerEventsClient(SERVER_EVENTS_URL, ["B"], {
    ...
    it ('Can unsubscribe from channels whilst connected', function(done) {
        ...
        let states = [];
        const client1 = new ServerEventsClient(SERVER_EVENTS_URL, ["A","B","C"], {
            handlers: { onMessage: e => msgs1.push(e) }, 
            onTick: run(states)
        }).start();
        const client2 = new ServerEventsClient(SERVER_EVENTS_URL, ["B","C"], {
            handlers: { onMessage: e => msgs2.push(e) }, 
            onTick: run(states)
        }).start();
    ...
    it ('Does fire multiple listeners for custom trigger', done => {
        ...
        const client2 = new ServerEventsClient(SERVER_EVENTS_URL, ["*"], {
            onTick: run(states)
        }).start();
    ...
    it ("Does raise Error Messages", done => {
        ...
        const client1 = new ServerEventsClient('http://chat.servicestack.invalid', ["*"], {
            onException: (e) => {
                expect(e).to.exist;
                errors.push(e);
            },
    ...
    it ('Does create EventSource instance withCredentials', () => {
        let client = new ServerEventsClient(SERVER_EVENTS_URL, ["*"], {})
            .start();
